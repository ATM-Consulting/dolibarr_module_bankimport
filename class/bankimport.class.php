...
pre($TLine,true);
	...
	private function doPayment(&$TLine, &$TAmounts, &$l_societe, $iFileLine, $fk_payment, $date_paye, $type='payment')
	{
	    ...
	    // Specific case of charge payment
		if($type=='payment_sc') {
		...
		if ($paiement_id > 0)
		{
			...
			// @TODO => $banklineid = 1 for PaymentSocialContribution. We need to get the real bank line id to have it properly reconciled after
